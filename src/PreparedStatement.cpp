...
bool PreparedStatement::addTTS(const char* text, const char* address, const char* CAcert){
	if(text == nullptr) return false;
	parts.push_back({ Part::TTS, new std::string(text), address, CAcert });
	return true;
}

bool PreparedStatement::addTTS(const String& text, const char* address, const char* CAcert){
	if(text.length() == 0) return false;
	stringTTS.push_back(new String(text));
	addTTS(stringTTS.back()->c_str(), address, CAcert);
}
...
void PreparedStatement::prepareWAV(void (*playCallback)(TTSError error, CompositeAudioFileSource* source)) {
	this->playCallback = playCallback;
	...
	for (const Part& part : parts) {
		if (part.type == Part::TTS) {
			TTSresults.push_back(nullptr);
			TextToSpeech.addJob({ *(std::string*)part.content, &TTSresults.back(), part.address, part.CAcert });
		}
	}
