...
void SpeechToIntentImpl::doJob(const STIJob& job){
	...
	IntentResult* result = identifyVoice(job.recordingFilename, job.address, job.CAcert);

	*job.result = result;
}

IntentResult* SpeechToIntentImpl::identifyVoice(const char* filename, const char* address, const char* CAcert){
