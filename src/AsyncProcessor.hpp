...
protected:
	AsyncProcessor(const char* name, uint16_t queueSize = 1, const char* address = "", const char* CAcert = "");
...
template<typename T>
AsyncProcessor<T>::AsyncProcessor(const char* name, uint16_t queueSize, const char* address, const char* CAcert) : task(name, taskFunc, 8192, this), semaphore(queueSize) {
	task.start(1, 0);
}
